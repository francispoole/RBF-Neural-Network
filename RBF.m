...
function x = preProcess(x, option)
    ...
    if option == 2
        ...
        [U,S,V] = svd(Sigma); %Eigenvectors and eigenvalues of covariance matrix
        mu = mean(x); %Mean
        x = ((S^(-1/2) * (U.') * (x-(ones(size(x,1),1)*mu)).')).'; %Need to tidy!
    end
    ...
    if option == 3
        Sigma = cov(x);
        [U, S, V] = svd(Sigma);
        U = U(:,1:end-1); %Reduce dimensionality of eigenvectors
        x = (U.' * x.').';
    end
end


%Returns centres c and co-variance Sigma of data x using K-Means
function [c, Sigma] = getCentres(x, M, Sigma)
    N = size(x,1); %No of training datapoints

    %--Step 1 - Assign each datapoint x to random subset y of size M
    y = ceil(rand(N,1)*M);

    %--Step 4 - Loop till no further reassignments
    reassigning = true;
    while reassigning

        %--Step 2 - Generate mean vectors of each subset
        c = zeros(M,size(x,2));
        for m = 1:M %For each subset m
            c(m,:) = mean(x(any(eq(y,m),2),:),1); %Get mean vector of subset m
        end

        %--Step 3 - Reassign data to closest centre
        distMatrix = zeros(N,M);
        for n = 1:N
            for m = 1:M
                distMatrix(n,m) = norm(x(n,:)-c(m,:));
            end
        end
        [~,yNew] = min(distMatrix,[],2);
        reassigning = any(yNew ~= y);
        y = yNew;
    end

    %-- Step 5 - Calculate covariances of datapoints of each subset
    if strcmp(Sigma,'cov') || any(size(Sigma) > 1)
        Sigma = zeros(size(x,2),size(x,2),M);
        for m = 1:M %For each subset m
            %Turns scalar covariance into diagonal matrix
            Sigma(:,:,m) = eye(size(x,2)) * cov(x(any(eq(y,m),2),:)); %Covariance of datapoints (identity matrix used when degenerate case of 1 point in centre)
        end
        Sigma(:,:,any(isnan(Sigma(:,1,:)),1)) = []; %Remove empty covariances
    else
        Sigma = mean(pdist(c.')); %Average distance between centres
    end
    %-- Step 6 - Remove empty centres and their covariances
    c = c(ismember(1:M,y)',:); %Remove empty centres
    

end

%Split data a into training set a and single validation set vA
function [x,v] = kFold(x, fold, foldSize, I)
    kStart = (fold-1)*foldSize + 1;
    kStop = fold * foldSize;
    ...
    v = x(I,:); %Validation set
    x(I,:) = []; %Remove validation set from training
end
...
function rbf = uniGaussianBasis(r,sigma)
    rbf = exp(-(r^2)./(2*(sigma^2))); %Univariate gaussian distribution
end

function rbf = multiGaussianBasis(r, Sigma)
    rbf = exp((-1/2 * r) * pinv(Sigma) * (r.'));%Multivariate gaussian distribution

end

function plotData(x,y,c)
    clf
    figure
    colorList = hsv(size(c,1));
    for m = 1:size(x,2) %For each variable
        subplot(7,2,m);
        hold on
        for n = 1:size(x,1) %For each datapoint n of variable m
            plot(x(n,m),x(n,end),'x','color', colorList(y(n),:))
        end
        for n = 1:size(c,1) %For each centre n of variable m
            plot(c(n,m),c(1,end),'o','color', colorList(n,:))
        end
        hold off
    end
end

function x = getTrainingData()
x = [0.00632  18.00   2.310  0  0.5380  6.5750  65.20  4.0900   1  296.0  15.30  396.90   4.98  24.00
...
function p = getPredictionData()
p = [ 0.03601 0.00  5.066  0 0.4629 6.5558 45.00 6.0278 1  291.0 17.50 389.27   7.37   
12.07703 0.00 18.100  0 0.6724 6.0222 89.80 2.0613 24 666.0 20.20  288.08 18.60  
 0.09743 0.00  4.418  0 0.4524 6.4730 49.30 5.1465 3  246.0 18.10 392.41   9.07 
 0.39397 0.00 10.746  0 0.5043 6.1331 60.80 4.4330 4  335.0 19.10 382.72  12.19  
 0.15014 0.00  8.198  0 0.5148 6.1047 60.10 4.0249 6  372.0 17.80 387.36  12.30  
 0.15042 0.00  5.034  0 0.4410 6.6478 40.10 6.4959 7  304.0 18.40 388.43   7.98 
 0.37141 0.00  5.932  0 0.4925 6.9508 67.30 4.4106 8  301.0 17.90 385.27   7.96  
 0.68786 0.00  9.759  0 0.5709 6.3932 69.20 3.6973 5  331.0 16.50 369.19  10.65];
end